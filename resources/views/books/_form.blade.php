...
{{-- 
    Variables esperadas: 
    - $book        Book|null   (null en create, modelo en edit) 
    - $categories  Collection  (todas las categorías) 
    - $authors     Collection  (todos los autores) --}} 
@php 
...
sm 
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6"> 
        <div> 
            <label for="publish_year" class="block text-sm font-medium text-slate
700"> 
                Año 
            </label> 
        ...
        <div> 
            <label for="pages" class="block text-sm font-medium text-slate
700">Páginas</label> 
        ...
        <div> 
            <label for="language" class="block text-sm font-medium text-slate
700">Idioma</label> 
            <select name="language" id="language" 
                    class="mt-1 block w-full rounded-md border-slate-300 shadow-sm 
                           focus:border-indigo-500 focus:ring-indigo-500"> 
                @php $idiomas = 
['Español','Inglés','Portugués','Francés','Alemán','Otro']; @endphp 
                @foreach ($idiomas as $idioma) 
                    <option value="{{ $idioma }}" 
                        {{ old('language', $book?->language) === $idioma ? 
'selected' : '' }}> 
                        {{ $idioma }} 
                    </option> 
                @endforeach 
    ...
    {{-- Descripción --}} 
    <div> 
        <label for="description" class="block text-sm font-medium text-slate
700">Descripción</label> 
...
old('description', $book?->description) }}</textarea> 
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6"> 
        <div> 
            <label for="cover_url" class="block text-sm font-medium text-slate
700">URL de portada</label> 
        ...
        <div> 
            <label for="total_copies" class="block text-sm font-medium text-slate
700"> 
            ...
            @foreach ($authors as $author) 
                <label class="flex items-center gap-2 text-sm text-slate-700"> 
                    <input type="checkbox" name="authors[]" value="{{ $author->id 
                        ...
                        {{ in_array($author->id, old('authors', $selectedAuthorIds)) 
? 'checked' : '' }} 
